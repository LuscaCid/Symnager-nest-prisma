...
export class ClientsControllers {
  ...
  async createClient(@Req() req: RequestCreateClient) {
    const {
      body,
      user: { user_id, email: userEmail, username },
    } = req;
    const { email, cpf } = body;
    console.log(userEmail, username, user_id);
    if (!userEmail || !username || !user_id)
      throw new UnauthorizedException('Usuário nao autenticado corretamente.');
    ...
    if (clientExists) {
      throw new UnauthorizedException({
        message: 'Cliente com este Cpf já cadastrado. Deseja continuar?',
      });
    } else if (clientExistsByEmail) {
      throw new UnauthorizedException({
        message: 'Cliente com este email já cadastrado. Deseja continuar?',
      });
    }
  ...
  async updateClient(@Body() updateDTO: UpdateClientDTO) {
    try {
      if (updateDTO.cpf) {
        const cpfAlreadyExits = await this.clientService.findByCpf(
          updateDTO.cpf,
        );
        if (cpfAlreadyExits)
          throw new UnauthorizedException(
            'Já existe um cliente com este cpf, deseja sobrescrever?',
          );
      }
      if (updateDTO.email) {
        const emailAlreadyInUse = await this.clientService.findByEmail(
          updateDTO.email,
        );
        if (emailAlreadyInUse)
          throw new UnauthorizedException(
            'Já existe um cliente com este E-mail, deseja sobrescrever?',
          );
      }
  ...
  async revokeClient(@Req() clientDeletedDTO: RevokeDTO) {
    ...
    const deleteResponse = await this.clientService.verifyPassword(
      tryPassword,
      user_id,
    );
