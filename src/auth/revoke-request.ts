...
export class VerifyUserInRequest implements NestInterceptor {
  ...
  intercept(context: ExecutionContext, next: CallHandler<any>): Observable<any> | Promise<Observable<any>>  {
    ...
    const datainsidetoken : UserInsideToken = this.jwtService.verify(token, {secret : authConfig.jwtSecret})
    
    const { email, user_id,username} = datainsidetoken
    if(!email || !user_id || !username )throw new UnauthorizedException("Usuario nao authenticado corretamente.")
    return next.handle()
  }
}